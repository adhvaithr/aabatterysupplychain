export const STATE_STYLES = {
  DETECTED: { bg: 'bg-neutral-800', fg: 'text-neutral-300', ring: 'ring-neutral-700' },
  ...
  RESOLVED: { bg: 'bg-neutral-800/70', fg: 'text-neutral-400', ring: 'ring-neutral-700' },
}
...
export function DCBadge({ code }) {
  return (
    <span
      className={`mono inline-flex items-center rounded-md px-2 py-0.5 text-[11px] tracking-wider ring-1 ring-inset ${DC_PILL[code] || 'bg-neutral-800 text-neutral-300 ring-neutral-700'}`}
    >
      {code}
    </span>
  )
...
export function ActionText({ action }) {
  ...
  if (action === 'WAIT')
    return <span className="mono text-[11px] tracking-widest text-[#22C55E]/70">WAIT</span>
  return <span className="mono text-[11px] tracking-widest text-neutral-500">MONITOR</span>
}
